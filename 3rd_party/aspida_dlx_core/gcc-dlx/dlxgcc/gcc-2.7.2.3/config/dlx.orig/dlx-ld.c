...
usage()
{
  fprintf(stderr,"Usage: dlx-ld [arguments]\n"
	  ...
	  "  [-T textstart]     Set start of text section.\n"
...
char	*argv[];
{
    ...
    int stacksize;
    unsigned int textstartloc = 0x1000;
    char *crt0 = NULL;
    char *pt; 
    Symbol *p;
	...
	if(!strncmp(argv[i],"-o",2)) {
	  ...
	  stacksize = strtol(argv[++i],NULL,0);
	    ...
	    /* Pick a starting location for the code.  */
	    textstartloc = strtol(argv[++i],NULL,0);
...
ranlib_file(char *fn)
{
  ...
  fclose(fp);
  fclose(info->fp);
  sprintf(buf,"/usr/bin/ar q %s .SYMTAB",fn);
  system(buf);
  unlink(".SYMTAB");
}
